...
def run_camera_server(port=5000, model_path=None):
    ...
    try:
        ...
        ip = get_local_ip()
        print(f"Starting camera server on {ip}:{port}")
        ...
        print(f"Camera server ready! Configure the robot to connect to: {ip}:{port}")
        processor.run()
        ...
        print(f"Error running camera server: {e}")
        sys.exit(1)

def run_robot_controller(server_ip, server_port=5000):
    """Run the robot controller on EV3"""
    try:
        from src.robot.robot_controllers import RobotController
        
        print(f"Starting robot controller, connecting to server at {server_ip}:{server_port}")
        ...
        print(f"Error running robot controller: {e}")
        sys.exit(1)
...
def main():
    ...
    if mode == 'camera':
        run_camera_server(port=args.port, model_path=args.model)
    elif mode == 'robot':
