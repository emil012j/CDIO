...
def move_robot_turn(angle_deg, speed=20):
    gyro.reset()  # Nulstil før ny drejning
    target_angle = angle_deg

    if target_angle > 0:
        tank.on(SpeedPercent(speed), SpeedPercent(-speed))  # Venstre
        while gyro.angle < target_angle:
            sleep(0.01)
    else:
        tank.on(SpeedPercent(-speed), SpeedPercent(speed))  # Højre
        while gyro.angle > target_angle:
            sleep(0.01)

    tank.off()
    print(f"Turned {target_angle} degrees using gyro (actual: {gyro.angle}°)")
