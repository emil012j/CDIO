...
while True:
    ...
    # Find positioner
    robot_pos, balls = find_positions(frame)
    if not robot_pos or not balls:
        print("Ingen robot eller bolde fundet!")
        continue
    
    # Vælg nærmeste bold
    target = min(balls, key=lambda t: np.linalg.norm(np.array(t)-np.array(robot_pos)))
    
    # Brug calculate_movement funktion
    direction = calculate_movement(robot_pos, target)
    ...
    cv2.circle(frame, robot_pos, 10, (0,255,0), -1)  # Robotten (grøn)
